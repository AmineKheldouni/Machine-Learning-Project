...
class LearnCrowd:
    ...
    def grad_likelihood_2(self, Pt, X, Y, model, alpha, beta, gamma, w):
        ...
        grad_lh_w = np.sum(mat3,axis=0) #Taille d,T
    ...
    def fit(self, X, Y, model="Bernoulli", eps = 10**(-7), max_iter=500):
        N = X.shape[0]
        d = X.shape[1]
        T = Y.shape[1]
        ...
        while ( normGrad >= 0.005 and (np.linalg.norm(self.alpha-alphaNew)**2 + (self.beta-betaNew)**2 >= eps or cpt_iter < max_iter)):
        # while (np.linalg.norm(self.alpha-alphaNew)**2 + (self.beta-betaNew)**2 >= eps or cpt_iter < max_iter):
            cpt_iter+=1
    ...
    def predict(self, X, seuil):
        ...
        return labels_predicted.ravel().astype(int)
...
def learn_cas_unif_x():
    ...
    noise_truth=0.0 #bruit sur l'attribution des vrais labels gaussiens sur les données 2D (on pourrait aussi jouer sur ecart-type gaussienne avec sigma)
    modele= "Bernoulli"
